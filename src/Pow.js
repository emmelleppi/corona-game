...
function Pow(props) {
  ...
  const [positionFactor, setPositionFactor] = useState(Math.random() * (Math.random() > 0.5 ? 1 : -1))
  ...
  return (
    <sprite {...props} scale={_scale} visible={visible} position={[position[0] + positionFactor, position[1] + positionFactor, position[2]]}>
        <spriteMaterial
            attach="material"
            color={0xffffff}
            transparent
