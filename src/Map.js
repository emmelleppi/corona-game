...
function ConvexPolyBody(props) {
  ...
  const [mapbody] = useConvexPolyhedron(() => ({
    ...
    collisionFilterMask:
      COLLISION_GROUP.BODY | COLLISION_GROUP.TILES | COLLISION_GROUP.CORONA,
  }));
  ...
  return (
    <mesh
      ...
      material={material}
      geometry={geometry}
      userData={{ type: COLLISION_GROUP.TILES }}
    />
  );
}
...
function Map() {
  ...
  useEffect(
    () => void group.current.children.forEach((mesh) => void addMapItem(mesh)),
    [group, addMapItem]
  );
